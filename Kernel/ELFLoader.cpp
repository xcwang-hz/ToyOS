...
bool ELFLoader::layout()
{
    ...
    m_image.for_each_program_header([&] (const ELFImage::ProgramHeader& program_header) {
        ...
        // }
// 1. Calculate physical address (Linear = Physical)
        uint8_t* dest_ptr = (uint8_t*)program_header.laddr().get();
        
        // 2. Calculate source offset in the ELF file buffer
        const uint8_t* src_ptr = reinterpret_cast<const uint8_t*>(m_image.raw_data(program_header.offset()));

        // This replaces both map_section_hook and alloc_section_hook logic
        memcpy(dest_ptr, src_ptr, program_header.size_in_image());

        // 4. Handle BSS (Zero out uninitialized data)
        // If memory size is larger than file size, the rest is BSS.
        if (program_header.size_in_memory() > program_header.size_in_image()) {
            size_t bss_size = program_header.size_in_memory() - program_header.size_in_image();
            memset(dest_ptr + program_header.size_in_image(), 0, bss_size);
        }        
    });
